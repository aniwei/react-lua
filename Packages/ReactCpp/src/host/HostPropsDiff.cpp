...
std::string coerceTextValue(jsi::Runtime& rt, const jsi::Value& value) {
  ...
  if (value.isNumber()) {
    ...
    // Trim trailing zeros similar to JS toString behaviour for integers.
...
bool HostPropsDiffResult::hasChanges() const {
...
HostPropsDiffResult diffHostProps(
  ...
  bool isEvent = isEventPropKey(key);
